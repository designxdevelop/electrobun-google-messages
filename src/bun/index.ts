...
const PREFIX_UNREAD_COUNT_REGEX = /^\((\d+)\)\s*/;
const SUFFIX_UNREAD_COUNT_REGEX = /\s\((\d+)\)\s*$/;

ApplicationMenu.setApplicationMenu([
  {
    submenu: [{ label: "About Messages", action: "app-about" }, { type: "separator" }, { role: "quit" }],
  },
  {
    label: "File",
    submenu: [{ role: "close" }],
...
const parseUnreadCount = (title: string): number => {
  const prefixMatch = title.match(PREFIX_UNREAD_COUNT_REGEX);
  if (prefixMatch) {
    return Number(prefixMatch[1]);
  }

  const suffixMatch = title.match(SUFFIX_UNREAD_COUNT_REGEX);
  if (suffixMatch) {
  ...
  try {
    ...
    if (unreadCount > lastUnreadCount) {
      ...
      Utils.showNotification({
        title: "Messages",
        body: newMessages === 1 ? "You have a new message." : `You have ${newMessages} new messages.`,
      });
    }
...
mainWindow.webview.setNavigationRules(["^*", "*://messages.google.com/*", "*://accounts.google.com/*", "*://*.google.com/*", "^http://*"]);
